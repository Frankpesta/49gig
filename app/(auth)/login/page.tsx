...
export default function LoginPage() {
  ...
  const [freelancerLoginIntent, setFreelancerLoginIntent] = useState(false);

  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      setFreelancerLoginIntent(
        new URLSearchParams(window.location.search).get("intent") === "freelancer"
      );
    } catch {
      setFreelancerLoginIntent(false);
    }
  }, []);
          ...
          <>
            <div className="space-y-1.5">
              <Button
                type="button"
                variant="outline"
                className="w-full h-11 rounded-lg text-sm font-medium border-border/60 bg-muted/30 hover:bg-muted/50 disabled:opacity-85"
                aria-disabled={freelancerLoginIntent ? true : undefined}
                title={
                  freelancerLoginIntent
                    ? "Google sign-in is not available for freelancer accounts."
                    : undefined
                }
                disabled={isLoading || isGoogleLoading || freelancerLoginIntent}
              ...
              {freelancerLoginIntent && (
                <p className="text-xs text-muted-foreground text-center">
                  Freelancer accounts: sign in with email and password. Google is not available for this path.
                </p>
              )}
            </div>
