"use client";

import { useRouter } from "next/navigation";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import {
  Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { useOAuth } from "@/hooks/use-oauth";
import { AuthTwoColumnLayout } from "@/components/auth/auth-two-column-layout";
import { signupFeatures } from "@/components/auth/auth-icons";

const authCardClass =
  "rounded-xl border border-border/60 bg-card shadow-lg";

export default function SignupPage() {
  const router = useRouter();
  const { signInWithGoogle } = useOAuth();

  const handleRoleSelect = (role: "client" | "freelancer") => {
    router.push(`/signup/${role}`);
  };

  return (
    <AuthTwoColumnLayout
      leftTitle="Join 49GIG today"
      leftDescription="Start your journey in a high-trust freelance marketplace. Connect with verified talent and trusted clients."
      features={signupFeatures}
      badge="Join a high-trust talent marketplace"
      heading="Create your account"
      subline="Choose how you want to use 49GIG."
    >
      <Card className={authCardClass}>
        <CardHeader className="space-y-1 px-6 sm:px-8 pt-6 sm:pt-8 pb-4">
          <CardTitle className="text-xl font-semibold">I am a</CardTitle>
          <CardDescription>Select your role to continue</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 px-6 sm:px-8 pb-6 pt-0">
          <button
            type="button"
            onClick={() => handleRoleSelect("client")}
            className="w-full flex items-start gap-4 p-4 rounded-xl border-2 border-border/60 bg-muted/20 hover:border-primary/40 hover:bg-primary/5 transition-all text-left"
          >
            <div className="flex-1 space-y-1">
              <div className="font-semibold text-foreground">Client</div>
              <div className="text-sm text-muted-foreground">
                I want to hire freelancers for my projects
              </div>
            </div>
          </button>

          <button
            type="button"
            onClick={() => handleRoleSelect("freelancer")}
            className="w-full flex items-start gap-4 p-4 rounded-xl border-2 border-border/60 bg-muted/20 hover:border-primary/40 hover:bg-primary/5 transition-all text-left"
          >
            <div className="flex-1 space-y-1">
              <div className="font-semibold text-foreground">Freelancer</div>
              <div className="text-sm text-muted-foreground">
                I want to work on projects and get paid
              </div>
            </div>
          </button>
        </CardContent>
        <CardFooter className="flex flex-col gap-5 border-t border-border/60 px-6 sm:px-8 py-6">
          <div className="relative w-full">
            <div className="absolute inset-0 flex items-center">
              <span className="w-full border-t border-border/60" />
            </div>
            <div className="relative flex justify-center">
              <span className="bg-card px-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                Or continue with
              </span>
            </div>
          </div>
          <div className="grid grid-cols-1 gap-3 w-full">
            <Button
              type="button"
              variant="outline"
              className="w-full h-11 rounded-lg text-sm font-medium border-border/80"
              onClick={() => signInWithGoogle("client")}
            >
              <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
              </svg>
              Sign up as Client with Google
            </Button>
            <Button
              type="button"
              variant="outline"
              className="w-full h-11 rounded-lg text-sm font-medium border-border/80"
              onClick={() => signInWithGoogle("freelancer")}
            >
              <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
              </svg>
              Sign up as Freelancer with Google
            </Button>
          </div>
          <p className="text-center text-sm text-muted-foreground">
            Already have an account?{" "}
            <Link href="/login" className="font-semibold text-primary hover:underline">
              Sign in
            </Link>
          </p>
        </CardFooter>
      </Card>
    </AuthTwoColumnLayout>
  );
}
